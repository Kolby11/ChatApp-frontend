...
function ChatPreview({ chat, openedChat, setOpenedChat }: Props) {
  ...
  useEffect(() => {
    if (openedChat === chat._id && notificationCount > 0) {
  ...
  return (
    <div className={`${openedChat === chat._id ? 'bg-secondary' : 'bg-black'} flex justify-between items-center h-14 px-4 py-2 text-white`}>
